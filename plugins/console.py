...
"""
    ...
    def on_key_activate(self, key, key_id):
    ...
    def do_initialize(self):
        self.window = _make_window(self.start_script)
        self.window.connect("destroy", self.on_window_destroy)
        self.item = GeanyGI.UiUtils.image_menu_item_new(Gtk.STOCK_EXECUTE, "Open python console")
        self.item.connect("activate", self.on_item_click)
        GeanyGI.Data.instance().widgets.tools_menu.append(self.item)
        self.item.show_all()

        self.keys = GeanyGI.KeyGroup.new("python_console", "Python Console", 1)
        self.keys.add_keybinding(GeanyGI.KeyBinding.new("open_console", "Open Console Window", self.on_key_activate), 0)
        pass
