...
void configuration_save_session_files(GKeyFile *config)
{
	...
	{
		g_snprintf(key, sizeof(key), "notebook%d", ++k);
		npage = gtk_notebook_get_current_page(notebook);
		g_key_file_set_integer(config, key, "current_page", npage);

		/* store the filenames in the notebook tab order to reopen them the next time */
		max = gtk_notebook_get_n_pages(notebook);
		for (i = 0; i < max; i++)
		{
			doc = notebook_get_document_from_page(notebook, i);
