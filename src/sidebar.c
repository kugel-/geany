...
static void tree_copy_recursive(GtkTreeIter *parent_old, GtkTreeIter *parent_new)
{
	...
	// expanded = gtk_tree_view_row_expanded(GTK_TREE_VIEW(tv.tree_openfiles), path);
	gtk_tree_path_free(path);
...
/**
 * Returns the position of dir delimiter where pathes don't match
 * */
